...
- [~] **16.5 — Parité rendu** : porter le vrai `Renderer`/`GIVolume`/features sur le backend web.
  ...
  - [ ] 16.5.b — Compiler la stack de rendu sous Emscripten : backends web pour `Mesh`/`Material`/`ResourceManager` (set 1 = BindGroup web, pas de bindless), stubs ImGuiLayer/GpuProfiler (desktop-only), sélection `Rhi.hpp` → RhiWeb sous `SAIDA_RHI_WEBGPU`, cible CMake web élargie.
  - [ ] 16.5.c — Parité feature par feature : shadows → DDGI (hoister le compute particules hors de la passe HDR, clearColorTexture non-zéro via pass) → particules GPU → post-process, une à la fois, validée dans le navigateur à chaque étape.
