...
| C2 | Collaboration Gateway (Fastify WebSocket) | A2,A4 | 🔵 amorce : substrat de validation prêt. **Décision (2026-07-04)** : la gateway valide les SaidaOps entrantes via un **build WASM headless de l'authoring-core C++** chargé **en process** (`web/authoring/`, ne linke que `SaidaOp.cpp` : `parseSaidaOp`+`validateOpShape`, sans scène/GPU) — zéro duplication (le vrai C++), zéro latence (pas de sous-process), léger (~95 Ko wasm). Étend l'invariant 0.2 au serveur. Exposé `saida_validate_op`/`saida_known_op_types`/`saida_op_version`, testé Node (`smoke_test.mjs`). Reste : service Fastify WS lui-même |
