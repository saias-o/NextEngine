...
- [x] **Étape 16 — Export Web (WASM + WebGPU).** RHI compile-time (Vulkan desktop/XR + WebGPU), 33 shaders GLSL→SPIR-V→WGSL (naga, desktop identique au bit près), backend `rhi/webgpu/*` (émulation push constants, clears texture, layouts). Le **vrai `Renderer` tourne dans le navigateur** : shadows + DDGI + skybox + particules GPU + AO + bloom + tonemap, validé pixel par pixel, ~213 Ko brotli. Système de features sur web via `recordPrePass` (compute hoisté hors de la passe scène). Packaging : `web/build_web.sh` + `serve.py` (COOP/COEP) + brotli + `BuildExporter::exportWebBuild`. **Future** : KTX2/Basis, fetch/IDBFS streaming, MSAA web, water/outline web (→ [TODO.md](TODO.md)).
