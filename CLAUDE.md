...
- [~] **Étape 8 — Couche jeu.** Fondation large : split moteur/jeu, sérialisation JSON, undo/redo, contrat authoring, signaux typés, World persistant, autoloads, groups, instanciation runtime, timers/tweens, runtime standalone, storage JS et jeu témoin. **Cycle de vie des assets (chantier 3, cœur livré)** : textures + meshes `.obj` chargés/décodés par l'`AssetLoader` asynchrone (worker desktop, `pump()` web), fallbacks visibles, rebind matériaux et re-fit physique au ready, déchargement GPU réel par mark-and-sweep au `changeScene` (graveyard + freelists bindless/matériaux), `gpuResidentBytes` asserté stable sur 16 cycles par l'E2E desktop+web. **Manque** : preuve sur machine vierge, communication cross-node/autoload JS, player web UI, round-trip authoring sans perte, et la finition assets (budget LRU en cours de scène, rigs/anims dans le sweep) (→ [TODO.md](TODO.md), frictions dans [docs/WITNESS_GAME.md](docs/WITNESS_GAME.md)).
...
- [x] **Étape 10 — Animation System.** Implémentation large, du cooker au LLM : assets versionnés à diagnostics structurés (`.sclip` vues non destructives, `.sgraph` schéma 2 avec triggers/exit time/sync de phase, `.sretarget` schéma 2 avec corrections de rest pose + échelle, `.srig` sémantiques/hash, `.sseq` séquences multipistes déterministes) ; cooker + cache dérivé par hash ; kernel data-oriented ; palette GPU affine 3x4 commune Vulkan/WGSL ; Blend1D, layers/masks/additif, root motion, événements, LOD, retargeting, IK/look-at ; panneau éditeur et outils headless/MCP. `TimelinePropertyTrack` est désormais implémenté par réflexion. La gate produit reste partielle : le WitnessGame ne traverse pas encore une vraie séquence `.sseq` exportée et les bindings JS animation ne sont pas complets.
...
- [~] **Étape 12 — UI 2D (Screen & World Space).** Desktop largement implémenté : RmlUi + QuickJS, `WebCanvasNode` avec rendu CPU et hot-reload transactionnel. **Limite majeure** : le player WebGPU ne rend pas encore les nœuds UI, qui se dégradent en Node générique. Backend GPU/Vulkan, parité player web, navigation manette et localisation restent ouverts.
- [~] **Étape 13 — Intégration LLM Native.** MCP, réflexion/manifeste, signaux data-driven, outils code+validation et primitives FSM/Blackboard/Scénario existent. **Manque** : permissions/transactions/diff/rollback de sécurité, inspecteur behaviours réfléchis, world model, skills et agents autonomes (→ [TODO.md](TODO.md)).
- [~] **Étape 14 — XR / OpenXR.** OpenXR SDK, intégration Vulkan/Engine, multiview, controllers et hand tracking skeletal `XR_EXT_hand_tracking` sont implémentés. **SaidaXRTK** couvre interaction, locomotion et scaffolds anchors/passthrough. **Manque** : validation hardware reproductible, MSAA multiview, ImGui overlay, Renderer DRY refactor et vrais anchor backends (→ [TODO.md](TODO.md)).
- [~] **Étape 15 — Build & Release Windows.** Export-template, runtime dédié, packager, UI Build Settings et version/métadonnées/icône existent. **Manque pour une release publique** : preuve Build UI sur machine vierge, archive/installeur et signature, crash logs, validation des DLL, provenance/SBOM et rollback ; LTO reste une optimisation secondaire (→ [TODO.md](TODO.md)).
- [x] **Étape 16 — Renderer Web (WASM + WebGPU).** RHI compile-time, shaders WGSL, backend WebGPU et vrai Renderer dans le navigateur existent ; BeachDemo sert de fixture historique. Le player gameplay est distinct et reste partiel (UI, input/capabilities, contenu corrompu), tout comme l'authoring web (registre de types/snapshot). Ne pas lire cette case comme « parité produit Web terminée ». **Future** : KTX2/Basis, fetch/IDBFS streaming, MSAA web (→ [TODO.md](TODO.md)).
...
## Décision scripting (prise et implémentée, API à compléter)
