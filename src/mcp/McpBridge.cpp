...
json toolListAnimationAssets(const ToolCtx& ctx, const json&) {
...
json toolInspectAnimationAsset(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
...
json toolValidateAnimationAsset(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
    const std::string abs = resolveToolPath(ctx, args["path"].get<std::string>()).absolute;
    std::vector<AssetDiagnostic> diags;
...
json toolCreateClipView(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
...
json toolCreateAnimationGraph(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
...
json toolCreateLocomotionGraph(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
...
json toolCreateAnimationSequence(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
...
json patchAnimationGraph(const ToolCtx& ctx, const json& args,
                         const std::function<void(json&)>& mutate) {
    if (!args.contains("path")) fail("missing 'path'");
    const std::string abs = resolveToolPath(ctx, args["path"].get<std::string>()).absolute;
    const std::string text = slurp(abs);
...
json toolSetGraphParameter(const ToolCtx& ctx, const json& args) {
    if (!args.contains("name")) fail("missing 'name'");
...
json toolAddAnimationState(const ToolCtx& ctx, const json& args) {
...
json toolAddAnimationTransition(const ToolCtx& ctx, const json& args) {
...
Animator* findPreviewAnimator(const ToolCtx& ctx, const json& args) {
    if (args.contains("id")) {
        Node* node = requireNode(ctx, args);
        while (node) {
...
json toolPreviewAnimationAsset(const ToolCtx& ctx, const json& args) {
    if (!args.contains("path")) fail("missing 'path'");
    const std::string abs = resolveToolPath(ctx, args["path"].get<std::string>()).absolute;
...
json toolList() {
    ...
    tools.push_back(tool("list_animation_assets",
        "Animation assets of the project: sources (.gltf/.glb/.bvh) and authored .sclip/.sgraph/.sretarget/.srig/.sseq.", obj({})));
    tools.push_back(tool("inspect_animation_asset",
    ...
    tools.push_back(tool("create_locomotion_graph",
        ...
        obj({{"path", str()}, {"idle", str()}, {"walk", str()}, {"run", str()},
             {"parameter", str()}}, {"path", "idle", "walk"})));
    tools.push_back(tool("create_animation_sequence",
        "Create a .sseq multi-track sequence (animation/event/property tracks). Validates before writing.",
        obj({{"path", str()}, {"sequence", json{{"type", "object"}}}}, {"path", "sequence"})));
    tools.push_back(tool("set_graph_parameter",
        "Add or update a typed parameter of a .sgraph. {path, name, type?, default?}.",
        obj({{"path", str()}, {"name", str()}, {"type", str()}}, {"path", "name"})));
    tools.push_back(tool("add_animation_state",
        "Add or replace a state of a .sgraph. {path, state:{name,play,loop?,speed?}, clipKey?} — clipKey declares the played alias if new.",
        obj({{"path", str()}, {"state", json{{"type", "object"}}}, {"clipKey", str()}}, {"path", "state"})));
    tools.push_back(tool("add_animation_transition",
        "Append a transition to a .sgraph. {path, transition:{from,to,crossfade?,exitTime?,syncPhase?,when?}}.",
        obj({{"path", str()}, {"transition", json{{"type", "object"}}}}, {"path", "transition"})));
    tools.push_back(tool("preview_animation_asset",
        "Play a .sclip or apply a .sgraph on an Animator ({id} or the first one in the scene).",
        obj({{"path", str()}, {"id", str()}}, {"path"})));
    return tools;
}
...
json McpBridge::callTool(EditorUI& ui, const std::string& name, const json& args) {
    ...
    if (name == "list_animation_assets") return toolListAnimationAssets(ctx, args);
    if (name == "inspect_animation_asset") return toolInspectAnimationAsset(ctx, args);
    if (name == "validate_animation_asset") return toolValidateAnimationAsset(ctx, args);
    if (name == "create_clip_view") return toolCreateClipView(ctx, args);
    if (name == "create_animation_graph") return toolCreateAnimationGraph(ctx, args);
    if (name == "create_locomotion_graph") return toolCreateLocomotionGraph(ctx, args);
    if (name == "create_animation_sequence") return toolCreateAnimationSequence(ctx, args);
    if (name == "set_graph_parameter") return toolSetGraphParameter(ctx, args);
    if (name == "add_animation_state") return toolAddAnimationState(ctx, args);
    if (name == "add_animation_transition") return toolAddAnimationTransition(ctx, args);
    if (name == "preview_animation_asset") return toolPreviewAnimationAsset(ctx, args);
